...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `network-mode` | string | `"dhcp"` | Set the target machine network mode. One of: `"dhcp"`, `"static"`, `"ipv6-slaac"`, `"offline"`. |
| `iso-network-mode` | string | `"dhcp"` | Set the installer network mode. One of: `"dhcp"`, `"static"`, `"ipv6-slaac"`, `"offline"`. |

Static IP network options (only when `network-mode = "static"`):

| Field | Type | Description |
|-------|------|-------------|
| `network-interface` | string | Name of the interface. If not set, defaults to all ethernet interfaces. |
| `network-ip` | string | Static IP to set in CIDR format. |
| `network-gateway` | string | IP of the gateway. |
| `network-dns` | string | Optional. Comma-separated DNS server IPs. |
| `network-domain` | string | Optional. Search/DNS domain. |

Installer static IP network options (only when `iso-network-mode = "static"`):

| Field | Type | Description |
|-------|------|-------------|
| `iso-network-interface` | string | Name of the interface. If not set, defaults to all ethernet interfaces. |
| `iso-network-ip` | string | Static IP to set in CIDR format. |
| `iso-network-gateway` | string | IP of the gateway. |
| `iso-network-dns` | string | Optional. Comma-separated DNS server IPs. |
| `iso-network-domain` | string | Optional. Search/DNS domain. |
...
Set a fixed recovery passphrase:

```toml
recovery-passphrase = "correct-horse-battery-staple"
```
...
| `recovery-passphrase` | string | _(autogenerated)_ | Recovery passphrase. When `disk-encryption` is not `"none"`, sets this as a password keyslot in the system LUKS partition. Must be at least 25 characters, printable ASCII only (no whitespace). |
| `save-recovery-keys` | boolean | `true` | Save recovery keys to recovery-keys.txt on the BESCONF partition. Each line will contain: passphrase, root partition UUID, machine serial. |
